...
public class LinkFunction implements Function, ServerExtension {
    ...
    private void saveProperties(String dictionary, String word, String dictName, Context context) {
        try {
            ...
            HttpURLConnection connection = getConnection(dictionary + "," + word, dictName, credentials, location);
            InputStream content = connection.getInputStream();
            BufferedReader in = new BufferedReader(new InputStreamReader(content));
            LOGGER.info("Save word: {} ; response: {}", word, in.readLine());
        } catch (IOException e) {
            throw new WebServiceException("Cannot connect to Sonar for updating dictionary." + e.getMessage());
        }
    }
